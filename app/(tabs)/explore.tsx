	...
	return (
		<ParallaxScrollView headerBackgroundColor={{ light: "#D0D0D0", dark: "#353636" }} headerImage={<Ionicons size={310} name="code-slash" style={styles.headerImage} />}>
			<ThemedView style={styles.titleContainer}>
				<ThemedText type="title">User Form</ThemedText>
			</ThemedView>
			...
			<View style={{ flex: 1 }}>
				...
				<TextInput style={{ height: 40 }} placeholder="Abc.." value={user.first_name} onChangeText={(text) => setUser({ ...user, first_name: text })} />
			</View>
			<View style={{ flex: 1 }}>
				<Text>Enter Last Name : </Text>
				<TextInput style={{ height: 40 }} placeholder="Xyz.." value={user.last_name} onChangeText={(text) => setUser({ ...user, last_name: text })} />
			</View>
			<View style={{ flex: 1, flexDirection: "row", gap: 5 }}>
				...
				<View style={{ width: "33%" }}>
					<Button
						...
						color={"skyblue"}
						onPress={() => {
							setUser({ first_name: "", last_name: "", full_name: "" });
						}}
					/>
				</View>
				<View style={{ width: "33%" }}>
			...
			<View style={{ flex: 1 }}>{showName && <Text>Your Full Name is : {user.full_name} </Text>}</View>
		</ParallaxScrollView>
	);
}
