	...
	return (
		<ParallaxScrollView headerBackgroundColor={{ light: "#A1CEDC", dark: "#1D3D47" }} headerImage={<Image source={require("@/assets/images/partial-react-logo.png")} style={styles.reactLogo} />}>
			...
			<ThemedView style={styles.stepContainer}>
				...
				<ThemedText>
					When you're ready, run <ThemedText type="defaultSemiBold">npm run reset-project</ThemedText> to get a fresh <ThemedText type="defaultSemiBold">app</ThemedText> directory. This will move the current <ThemedText type="defaultSemiBold">app</ThemedText> to{" "}
					<ThemedText type="defaultSemiBold">app-example</ThemedText>.
				</ThemedText>
			</ThemedView> */}
			<View style={{ flex: 1 }}>
				<Text>Enter First Name :</Text>
				<TextInput style={{ height: 40 }} placeholder="Abc.." onChangeText={(text) => setFirstName(text)} />
			</View>
			<View style={{ flex: 1 }}>
				<Text>Enter Last Name : </Text>
				<TextInput style={{ height: 40 }} placeholder="Xyz.." onChangeText={(text) => setLastName(text)} />
			</View>
			<Button
				title={showName ? "Hide" : "Show"}
				color={showName ? "grey" : "skyblue"}
				onPress={() => {
					setShowName(!showName);
				}}
			/>
			<View style={{ flex: 1 }}>
				<Text>Your Full Name is : {showName && fullName} </Text>
				<Text>{showName} </Text>
			</View>
		</ParallaxScrollView>
	);
